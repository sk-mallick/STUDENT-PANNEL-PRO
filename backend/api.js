...
const BACKEND_URL = 'https://script.google.com/macros/s/AKfycbxpKT6B6UYNNiLfzzT8vD40B7PGKrhbWA2TSS8mCkqSXl9j6VyBsiyLla31IBUKB8w5Vw/exec';
...
export const API = {
     ...
     * Returns { success: true } if valid, { success: false, reason: 'another_device' } if not.
     */
    async checkSession(studentId, sessionToken) {
        if (!BACKEND_URL) return { success: true }; // skip if no backend
        ...
        try {
            ...
            if (!res.ok) return { success: true }; // allow on HTTP error
            return await res.json();
        } catch {
            // Network error — allow offline, don't block the user
            return { success: true };
    ...
    /**
     * Fetch authoritative dashboard data computed server-side.
     * Returns { success, data: { overview, recentActivities, subjectBreakdown } }
     */
     ...
     * Returns { success, studentId, message } on success.
     */
    async register(name, email, password) {
        ...
        try {
            ...
            console.warn('[API] Registration failed:', err.message);
            return { success: false, error: err.message };
        }
    },

    /**
     * Server-side logout: clears activeSessionToken in the REGISTRATION sheet.
     * Fire-and-forget — we don't block the user if this fails.
     */
    async serverLogout(studentId, sessionToken) {
        if (!BACKEND_URL || !studentId || !sessionToken) return;
     ...
     */
    async syncResult(subject, level, set, score, total, timeTaken) {
        if (!BACKEND_URL) {
            return { success: false, reason: 'no_backend_configured' };
        }
        ...
        const payload = {
            ...
            studentName: getStudentName() || 'Anonymous',
            subject,
            level,
            set,
            score,
